...
const usersConnection = require('../db').db().collection('users');
...
User.prototype.validate = function() {
    return new Promise(async (resolve, reject) => {
        ...
        if(this.data.username.length > 2 && this.data.username.length < 31 && validator.isAlphanumeric(this.data.username)) {
            let usernameExist = await usersConnection.findOne({username: this.data.username});
        ...
        if(validator.isEmail(this.data.email)) {
            let emailExist = await usersConnection.findOne({email: this.data.email});
...
User.prototype.register = function() {
    return new Promise(async (resolve, reject) => {
        ...
        if(!this.errors.length) {
            ...
            await usersConnection.insertOne(this.data);
            this.getAvatar();
            resolve();
        } else {
            reject(this.errors);
...
User.findByUsername = function(username) {
    return new Promise(function(resolve, reject) {
        ...
        usersConnection.findOne({username: username})
